...
def get_telegram_message(db: Session, message_id: int):
    return db.query(models.MedicalItem).filter(models.MedicalItem.id == message_id).first()
...
def create_telegram_message(db: Session, message: schemas.TelegramMessageCreate):
    db_message = models.MedicalItem(**message.dict())
    db.add(db_message)
    db.commit()
...
def update_telegram_message(db: Session, message_id: int, message: schemas.TelegramMessageUpdate):
    db_message = db.query(models.MedicalItem).filter(models.MedicalItem.id == message_id).first()
    if db_message:
